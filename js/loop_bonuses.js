(function () {

"use strict";

// ================================= LOOPS BONUSES
//
//     BONUSES 1:
//
//     Write a function to toggle all characters in a string: in a given string, toggle all characters to
// their opposite case. Special characters will not change.
//

    // var elements = ["a","B","c","1","2","#"];
    
    var str = "aBc12#";
//     Example output: "AbC12#"



    // function toggleAll(element) {
    //     var output;
    //     output = element.join("");
    //     return output
    // }
    
    function toggleAll(str) {
        for (var i = 0; i <= str.length; i += 1) {
            if (str)
        }
    }


    console.log(toggleAll(elements));


//     BONUS 2:
//
// Write a function that takes a given string appended with a number at the end.
// The program will check the length of the string to verify the string is equal to the number appended.
// If the number appended equals the string length, output "Yes". if the number appended does not equal the string length, output "No".
//
//         Example Input: "geek5"
//     Example Output: "Yes"
//
//     Example Input: "codingchallenge25"
//     Example Output: "No"



})();
